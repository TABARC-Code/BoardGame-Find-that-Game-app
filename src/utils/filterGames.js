export function filterGames(games, filters, favorites = []) {
  return games
    .filter(game => {
      ...
      // Weight (OR logic). Note: an empty selection must exclude every
      // game (nothing is selected), not fall through to "show all" -
      // `.includes` on an empty array is already always false, so no
