...
const OLD_CSV_PATH = process.argv[3] || '';
const OUTPUT_PATH = './public/data/games.json';

if (!CSV_PATH) {
  console.error('Usage: node scripts/convert-bg-info.js <path-to-bg_info.csv> [path-to-old-board_games.csv]');
  process.exit(1);
}
...
async function fetchGameDetails(gameId) {
  ...
  try {
    ...
    // Extract image from og:image meta tag. Note: BGG's og:image is a
    // large (~1200x630) signed CDN URL. Its hash/crop params are bound to
    // that exact size, so it cannot be rewritten into a smaller "__thumb"
    // variant after the fact (swapping the label alone returns the same
    // bytes; changing the crop dimensions invalidates the signature and
    // 400s). A real thumbnail requires pulling a separately-signed URL
    // from the page's image data, not derived from this one.
    ...
    // Best-effort mechanics extraction from the mechanics link list BGG
    // renders on the game page (/boardgamemechanic/<id>/<slug>). Falls
    // back to an empty list if the page doesn't match, same as before.
    const mechanicMatches = [...html.matchAll(/\/boardgamemechanic\/\d+\/([a-z0-9-]+)"[^>]*>([^<]+)</gi)];
    const mechanics = [...new Set(mechanicMatches.map(m => m[2].trim()))].slice(0, 8);
...
async function main() {
  ...
  // Data-quality guard: searchBGG() falls back to fuzzy title search when a
  // game isn't in the old ID map, which can resolve two different games
  // (e.g. a base game and its stand-alone expansion) to the same BGG page.
  // That produces a wrong "View on BoardGameGeek" link for one of them, so
  // surface it instead of silently shipping it.
  const gameIdCounts = new Map();
  for (const g of output) {
    gameIdCounts.set(g.game_id, (gameIdCounts.get(g.game_id) || 0) + 1);
  }
  const duplicateIds = [...gameIdCounts.entries()].filter(([, count]) => count > 1);
  if (duplicateIds.length > 0) {
    console.warn(`\nWarning: ${duplicateIds.length} game_id value(s) are shared by multiple entries (likely mismatched BGG search fallback):`);
    for (const [gameId] of duplicateIds) {
      const names = output.filter(g => g.game_id === gameId).map(g => g.uid);
      console.warn(`  game_id ${gameId}: ${names.join(', ')}`);
    }
  }
